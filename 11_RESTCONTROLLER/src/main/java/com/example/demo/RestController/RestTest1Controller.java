...
public class RestTest1Controller {
    ...
    @GetMapping(value = "/getText",produces= MediaType.TEXT_PLAIN_VALUE) //컨트롤러가 반환하는 **응답(Response)의 콘텐츠 타입(Content-Type)**을text/plain으로 설정하겠다는 의미
    public String t1(){
    ...
    public ResponseEntity< List<MemoDto> >t5(
        ...
        if(show){List<MemoDto> list = new ArrayList();
        for(long i=1;i<=100;i++){
            MemoDto tmp = new MemoDto(i,"내용-"+i,"작성자-"+i,LocalDateTime.now());
            list.add(tmp);
        }
//        return new ResponseEntity("데이터값","상태정보값"); HttpStatus.OK = 정상. 200이라는 값임.
//        return new ResponseEntity(list , HttpStatus.OK); // 이것말고
        return ResponseEntity.status(HttpStatus.OK).body(list); // 이렇게도 가능
    }
    return new ResponseEntity(null , HttpStatus.BAD_GATEWAY); // return이 null이면 이렇게. 자세한건 HTTP상태코드표 검색해서 확인.
    }
}
