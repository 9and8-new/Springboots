...
public class KakaoLoginController {
    ...
    public String getCode(String code){
        log.info("GET /kakao/getCode...." + code);
        this.code = code;
...
//        restTemplate.exchange("URL", "요청METHOD","entity","반환값 받아낼 자료형");
    ...
    public String main(Model model){
        ...
        // 요청 헤더 설정
        HttpHeaders header = new HttpHeaders();
        header.add("Authorization", "Bearer "+ kakaoTokenResponse.getAccess_token()); // 공백 띄어둬야함
        header.add("Content-Type", "application/x-www-form-urlencoded;charset=utf-8");
...
//        restTemplate.exchange("URL", "요청METHOD","entity","반환값 받아낼 자료형");
    ...
    //로그아웃(엑세스토큰만 만료 / 카카오서버에서는 로그인되어있어서 로그인페이지가면 자동 로그인됨.)
    @GetMapping("/logout1")
    @ResponseBody
    public void logout1(){
...
//        restTemplate.exchange("URL", "요청METHOD","entity","반환값 받아낼 자료형");
    ...
    // 카카오계정과 함께 로그아웃
    @GetMapping("/logout3")
//    @ResponseBody //비동기처리를 하면 안되니까 주석
