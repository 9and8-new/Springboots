...
import com.example.demo.Domain.Admin.Dto.MemoDto;
...
//                date = LocalDate.now();
                ...
                //format 확인(yyyy#MM#dd로 받기로함)
                text = text.replaceAll("#","-"); // #을 -로 변환
                date = LocalDate.parse(text, DateTimeFormatter.ofPattern("yyyy-MM-dd")); //date에 입력값없으면 현재날짜로 변경
            }
            setValue(date);
        }
    }
